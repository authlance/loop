...
Install `@authlance/cli` as a dev dependency in your application.

With yarn:

    yarn add @authlance/cli@next --dev

With npm:

    npm install @authlance/cli@next --save-dev
...
The following targets are supported: `browser`. By default `browser` target is used.
